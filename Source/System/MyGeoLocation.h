...
using myGeoETRS89 = std::tuple<int, ty, ty>;

template <std::floating_point ty>
using myGeoMercatorETRS89 = std::pair<ty, ty>;

template <std::floating_point ty>
inline std::ostream& operator << (std::ostream& out, myGeoMercatorETRS89<ty> const& data) {
...
template <std::floating_point ty>
struct TMyGeoLocation : public std::pair<ty, ty> {
   friend std::ostream& operator << (std::ostream& out, TMyGeoLocation<ty> const& data) {
      out << "(" << data.first << ", " << data.second << ")";
      return out;
      }

   friend void swap(TMyGeoLocation& lhs, TMyGeoLocation&rhs) noexcept {
      lhs.swap(rhs);
      }
   ...
   TMyGeoLocation(myGeoMercatorETRS89<ty> const& loc) {
   ...
   TMyGeoLocation& operator = (TMyGeoLocation const& ref) {
      this->first = ref.first;
      this->second = ref.second;
      return *this;
      }

   TMyGeoLocation& operator = (myGeoETRS89<ty> const& loc) {
      auto val = ConvertETRS89ToWGS84(loc);
      this->first  = val.first;
      this->second = val.second;
      return *this;
   ...
   TMyGeoLocation& operator = (myGeoMercatorETRS89<ty> const& loc) {
      auto val = ConvertMercatorETRS89ToWGS84(loc);
      this->first = val.first;
      this->second = val.second;
      return *this;
   ...
   void swap(TMyGeoLocation& rhs) noexcept {
...
public:
   ...
   static auto ConvertMercatorETRS89ToWGS84(myGeoMercatorETRS89<ty> const& mercator) {
